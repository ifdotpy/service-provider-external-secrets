...
import (
	...
	esov1alpha1 "github.com/openmcp-project/service-provider-external-secrets/api/v1alpha1"
)

// runWorkspaceTarget: the tenant workspace is the target. Enabling the service
// in an account installs one External Secrets Operator for that account on the
// platform cluster, acting on the workspace through a minted kubeconfig. The
// workspace binds the upstream ESO APIs through the same APIExport.
func runWorkspaceTarget(
	...
	mcMgr, err := mcmanager.New(cfg, provider, manager.Options{Scheme: scheme, Metrics: metricsServerOptions, HealthProbeBindAddress: probeAddr})
	if err != nil {
	...
	handler := &esoWorkspaceHandler{platform: platformCluster.Client(), providerCfg: cfg, podNamespace: podNamespace, providerName: providerName, log: log}
...
type esoWorkspaceHandler struct {
...
const (
	wsESONamespace  = "external-secrets"
	wsESOSA         = "external-secrets"
...
func (h *esoWorkspaceHandler) Ensure(ctx context.Context, ws workspace.Workspace) error {
	...
	repo := &sourcev1.OCIRepository{ObjectMeta: metav1.ObjectMeta{Name: "external-secrets", Namespace: nsName}}
	...
	values, err := json.Marshal(esoWorkspaceValues())
	if err != nil {
		return err
	}
	...
	if _, err := ctrl.CreateOrUpdate(ctx, h.platform, hr, func() error {
		hr.Spec = helmv2.HelmReleaseSpec{
			...
			Install:  &helmv2.Install{Remediation: &helmv2.InstallRemediation{Retries: 3}},
...
// esoWorkspaceValues: controllers on the platform cluster acting on the workspace.
// CRDs come from the APIBinding; webhook and cert-controller are off because a
// webhook registered in the workspace could not reach a Service on the
// platform cluster; platform RBAC is not needed.
func esoWorkspaceValues() map[string]any {
	env := []map[string]any{{"name": "KUBECONFIG", "value": kubeconfigMount + "/" + kubeconfigKey}}
	vols := []map[string]any{{"name": "workspace-kubeconfig", "secret": map[string]any{"secretName": "workspace-kubeconfig"}}}
	mounts := []map[string]any{{"name": "workspace-kubeconfig", "mountPath": kubeconfigMount, "readOnly": true}}
	return map[string]any{
		"installCRDs":       false,
		"crds":              map[string]any{"createClusterExternalSecret": false, "createClusterSecretStore": false, "createPushSecret": false},
		"rbac":              map[string]any{"create": false},
		"serviceAccount":    map[string]any{"create": true},
		"leaderElect":       true,
		"extraEnv":          env,
		"extraVolumes":      vols,
		"extraVolumeMounts": mounts,
		"webhook":           map[string]any{"create": false},
		"certController":    map[string]any{"create": false},
	}
}
